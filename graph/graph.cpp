...
void Graph::BuildReverseIndex() {
    ...
    for (ui i = 0; i < labels_count_; ++i) {
        std::sort(reverse_index_ + reverse_index_offsets_[i], reverse_index_ + reverse_index_offsets_[i + 1],
            [this](const VertexID u, const VertexID v) -> bool {
                return getVertexDegree(u) > getVertexDegree(v);
            });
    }
}
...
void Graph::buildTriangleInfo() {
    uint64_t triangle_count = 0;
    for (ui u = 0; u < vertices_count_; ++u) {
        ui u_degree;
        const VertexID* u_neighbors = getVertexNeighbors(u, u_degree);
        for (ui i = 0; i < u_degree-1; ++i) {
            VertexID u_n = u_neighbors[i];
            if (u >= u_n) continue;
            for (ui j = i+1; j < u_degree; ++j) {
                VertexID u_nn = u_neighbors[j];
                if (u >= u_nn) continue;
                if (checkEdgeExistence(u_n, u_nn)){
                    edge_triangle_vertices_[((uint64_t)u << 32) | u_n].push_back(u_nn);
                    edge_triangle_vertices_[((uint64_t)u << 32) | u_nn].push_back(u_n);
                    VertexID u_1=std::min(u_n, u_nn);
                    VertexID u_2=std::max(u_n, u_nn);
                    edge_triangle_vertices_[((uint64_t)u_1 << 32) | u_2].push_back(u);
                    triangle_count += 1;
                }                    
            }
        }
