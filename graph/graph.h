...
private:
    ...
    std::unordered_map<uint64_t, std::vector<VertexID>> edge_triangle_vertices_;
...
public:
    const std::vector<VertexID> & getTriangleVertices(const VertexID u, const VertexID v) const {
        VertexID u1=std::min(u,v);
        VertexID u2=std::max(u,v);
        uint64_t key = ((uint64_t)u1 << 32) | u2;
        auto it = edge_triangle_vertices_.find(key);
        static const std::vector<VertexID> empty;

        if (it != edge_triangle_vertices_.end()) {
            return it->second;
        }
        return empty;
    }
    ...
    const ui * getNeighborsByLabel(const VertexID id, const LabelID label, ui& count) const {
        size_t offset = (size_t)id * labels_count_ + label;
