...
void GenerateFilteringPlan::generateVSSimFilterPlan(const Graph *data_graph, const Graph *query_graph, TreeNode *&tree,
                                                   VertexID *&order) {
    ui query_vertices_num = query_graph->getVerticesCount();
    bool *visit = new bool[query_vertices_num]();
    bool *popped = new bool[query_vertices_num]();
    ui *score = new ui[query_vertices_num];
    ...
    order[0] = selectVSSimStartVertex(data_graph, query_graph, score);
    visit[order[0]] = true;
    ...
    while (begin < end) {
        std::sort(order + begin, order + end,
          [&](VertexID v1, VertexID v2) -> bool {
                LabelID l1 = query_graph->getVertexLabel(v1);
                LabelID l2 = query_graph->getVertexLabel(v2);
                if (l1 == l2) return query_graph->getVertexDegree(v1) > query_graph->getVertexDegree(v2);
                ui lf1 = query_graph->getLabelsFrequency(l1);
                ui lf2 = query_graph->getLabelsFrequency(l2);
                if (lf1 == lf2) return l1 < l2;
                return lf1 < lf2;
          });
        size_t cur_level_end = end;
        while (begin < cur_level_end) {
            VertexID parent = order[begin];

            begin += 1;
            popped[parent] = true;

            ui nbrs_count;
            const VertexID* nbrs = query_graph->getVertexNeighbors(parent, nbrs_count);
            for (ui i = 0; i < nbrs_count; ++i) {
                VertexID child = nbrs[i];

                if (!popped[child]) {
                    tree[parent].fn_[tree[parent].fn_count_++] = child;
                    tree[child].bn_[tree[child].bn_count_++] = parent;
...
VertexID GenerateFilteringPlan::selectVSSimStartVertex(const Graph *data_graph, const Graph *query_graph, ui *score) {
    VertexID start_vertex = 0;
    double min_rank = std::numeric_limits<double>::max();
    for (VertexID v = 0; v < query_graph->getVerticesCount(); ++v) {
        if (query_graph->get2CoreSize() > 0 && query_graph->getCoreValue(v) < 2) {
            continue;
        }
        LabelID l = query_graph->getVertexLabel(v);
        ui d = query_graph->getVertexDegree(v);
        ui cur_label_can_size;
        const VertexID *cur_label_cands = data_graph->getVerticesByLabel(l, cur_label_can_size);
        auto pos = std::lower_bound(
            cur_label_cands, cur_label_cands +  cur_label_can_size, d,
            [&](VertexID can, ui deg) -> bool { return data_graph->getVertexDegree(can) >= deg; });
        ui init_cand_size = pos - cur_label_cands;
        score[v] = init_cand_size;
        # ifdef VSSim_DEBUG
        ui init_cand_size_test = 0;
        for (ui i = 0; i < cur_label_can_size; i++) {
            ui degree = data_graph->getVertexDegree(cur_label_cands[i]);
            if (degree >= d) {
                init_cand_size_test++;
            }
        }
        assert(init_cand_size_test == init_cand_size);
        # endif
        double rank = init_cand_size / (double)d;
        if (rank < min_rank) {
            start_vertex = v;
            min_rank = rank;
        }
